...
public:
    ...
    int tabulation2(vector<vector<int>>& m){
        int n = m.size();

        for(int i = n-2; i>=0; i--){
            for(int j = 0; j<n; j++){

                int ans = m[i][j] + m[i+1][j];

                if(j>0) ans = min(ans, m[i][j]+m[i+1][j-1]);

                if(j<n-1) ans = min(ans, m[i][j]+m[i+1][j+1]);
